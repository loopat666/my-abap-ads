...
form set_salv_display .
  ...
  if go_alvtree_sflight is initial.
    ...
    go_alvtree_sflight->get_functions( )->add_function(
      exporting
        name     = `NEW`                  " ALV Function
        icon     = conv #( icon_new_task )
        text     = `New`
        tooltip  = `New`
        position = if_salv_c_function_position=>right_of_salv_functions                 " Positioning Function
    ).

*    go_alvtree_sflight->get_selections( )->set_se
...
*   Events.
    data(lo_event) = go_alvtree_sflight->get_event( ).

    go_salv_event_handler = new #( ).

    set handler go_salv_event_handler->on_added_function for lo_event.
